...
clear s_tan_vel s_x s_y s_z s_vx s_vy s_vz s_infallz s_l s_b s_pm_l s_pm_b s_Rhel s_Vlos s_R
stream_id = "179";
st = load(strcat("C:/Users/dpryd/Desktop/streamFind/mock_catalogues_final/AquariusC2_RRLS_pe_byIDstream/AquariusC2RRLS.id",stream_id,".pe.dat"));
st_info = load(strcat("C:/Users/dpryd/Desktop/streamFind/mock_catalogues_final/AquariusC2_RRLS_info/AquariusC2RRLS.id",stream_id,".info.dat"));

x_min = -100;	%limit the volume to calculate the average position for input to galpy
x_max = 100;
y_min = -100;
y_max = 100;
z_min = -100;
...
stream_x = st(:,6);	%position of particles in stream
stream_y = st(:,7);
stream_z = st(:,8);

stream_vx = st(:,9);	%velocities of particles in stream
stream_vy = st(:,10);
stream_vz = st(:,11);
...
stream_l = st(:,13);	%galactic longitude
stream_b = st(:,14);	% galactic latitude
stream_pm_l = st(:,16);		% proper motion in l including cos b term
stream_pm_b = st(:,17);
stream_Rhel = st(:,15);		%heliocentric distance
stream_Vlos = st(:,18);		% line of sight/radial velocity
stream_parallax = st(:,12);
...
%plot3(avg_x,avg_y,avg_z,"*","markersize",20,"color","r")
xlabel("X/kpc")
ylabel("Y/kpc")
zlabel("Z/kpc")
...
plot(avg_R,avg_Vlos,"*","markersize",20,"color","r")
xlabel("R/kpc")
ylabel("V_r/km/s")
...
plot(avg_pm_l, avg_tan_vel,"*","markersize",20,"color","r")
xlabel("mu_l/mas/yr")
ylabel("V_t/km/s")
