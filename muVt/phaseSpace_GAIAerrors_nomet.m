...
## 
## 
## This version plots with ERRORS in data.
## Use phaseSpace_GAIA.m for NON ERROR CONVOLVED data plotting.
##
...
for j=1:N_prog
	...
	x= data(:,19);		%spatial positions
	y = data(:,20);
	z = data(:,21);
	mul = data(:,29);	%proper motion in galactic longitude, mulcosb
	mub = data(:,30);	% proper motion in galactic latitude
	% v_x = data(:,22);	%velocities/km/s
	% v_y = data(:,23);
	% v_z = data(:,24);
	rhel = data(:,28);	%heliocentric distance/kpc
	parallax = data(:,25); % parallax
	mu = sqrt(mul.*mul + mub.*mub);	%overall proper motion
	tan_vel = 4.75*mu./parallax;	%tangential velocity of each star
	rel_err_mub = data(:,36);		%relative error in proper motion
	n=size(x);			%number of particles in each progenitor
	for i=1:n
		if(rhel(i) > r_min && rhel(i) < r_max && rel_err_mub(i) <= mub_err)	%only plot particles within these spatial ranges
			rad(end+1) = rhel(i); 
			% L_x(end+1) = mass*(y(i).*v_z(i) - z(i).*v_y(i));	%angular momentum components
			% L_y(end+1) = mass*(z(i).*v_x(i) - x(i).*v_z(i));
			% L_z(end+1) = mass*(x(i).*v_y(i) - y(i).*v_x(i));
...
% L2 = L_x.*L_x + L_y.*L_y + L_z.*L_z;	%total angular momentum squared
% L = sqrt(L2);
...
% figure 5
% scatter(L_z, L_perp)
% xlabel("L_z/kgkpckm/s","FontSize",20)
% ylabel("L_perp/kgkpckm/s","FontSize",20)
...
print(18,strcat("C:/Users/dpryd/Desktop/streamFind/Aquarius_plots_final/",filename,"_tanV.png"),"-dpng")
print(20,strcat("C:/Users/dpryd/Desktop/streamFind/Aquarius_plots_final/",filename,"_tanV_filtered.png"),"-dpng")


save(strcat("C:/Users/dpryd/Desktop/streamFind/muVt/density_fits/",filename,"_tanV_density"),"density")
save(strcat("C:/Users/dpryd/Desktop/streamFind/muVt/density_fits/",filename,"_tanV_filtered"),"mask")

save_fits_image(strcat("C:/Users/dpryd/Desktop/streamFind/muVt/density_fits/",filename,"_tanV_density.fits"), density)
save_fits_image(strcat("C:/Users/dpryd/Desktop/streamFind/muVt/density_fits/",filename,"_tanV_filtered.fits"), mask)
